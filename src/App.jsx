...
export default function App() {
  ...
  const [menuItems, setMenuItems] = useState(() => {
    const saved = localStorage.getItem('cb-menu-items');
    return saved ? JSON.parse(saved) : MENU_ITEMS;
  });

  const [itemVariants, setItemVariants] = useState(() => {
    const saved = localStorage.getItem('cb-item-variants');
    return saved ? JSON.parse(saved) : ITEM_VARIANTS;
  });

  const [orders, setOrders] = useState(() => {
    const saved = localStorage.getItem('cb-orders');
    return saved ? JSON.parse(saved) : [];
  });

  const [reviews, setReviews] = useState(() => {
    const saved = localStorage.getItem('cb-reviews');
    return saved ? JSON.parse(saved) : [];
  });
  ...
  useEffect(() => {
    localStorage.setItem('cb-menu-items', JSON.stringify(menuItems));
  }, [menuItems]);

  useEffect(() => {
    localStorage.setItem('cb-item-variants', JSON.stringify(itemVariants));
  }, [itemVariants]);

  useEffect(() => {
    localStorage.setItem('cb-orders', JSON.stringify(orders));
  }, [orders]);

  useEffect(() => {
    localStorage.setItem('cb-reviews', JSON.stringify(reviews));
  }, [reviews]);
  ...
  const handleSaveProduct = (productData) => {
    ...
    if (exists) {
      setMenuItems(prev => prev.map(item => item.id === productData.id ? productData : item));
      setCart(prev => prev.map(c => c.id === productData.id ? { ...c, name: productData.name, img: productData.img, price: productData.price } : c));
      showToast('Product updated successfully!', productData.img, 'success');
    } else {
      setMenuItems(prev => [...prev, productData]);
      showToast('Product added successfully!', productData.img, 'success');
    }
